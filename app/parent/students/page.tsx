...
export default function ParentStudentsPage() {
  return (
    <Suspense fallback={<StudentsLoading />}>
      <ParentStudentsContent />
    </Suspense>
  );
}

function ParentStudentsContent() {
  const router = useRouter();
  const searchParams = useSearchParams();
  const bookingMode = searchParams.get("mode") === "booking";
  const requestedCategory = searchParams.get("category")?.trim() || "";
  const requestedSubject = searchParams.get("subject")?.trim() || "";

  const bookingStudentParams = new URLSearchParams({ mode: "booking" });
  if (requestedCategory)
    bookingStudentParams.set("category", requestedCategory);
  if (requestedSubject) bookingStudentParams.set("subject", requestedSubject);
  const bookingNextPath = `/parent/students?${bookingStudentParams.toString()}`;

  function subjectsPath(studentId: string) {
    const params = new URLSearchParams({ studentId });
    if (requestedCategory) params.set("category", requestedCategory);
    if (requestedSubject) params.set("subject", requestedSubject);
  ...
  useEffect(() => {
    async function loadData() {
      ...
      if (!user) {
        const next = encodeURIComponent(
          bookingMode ? bookingNextPath : "/parent/students",
        );
      ...
      const { data: levelRows, error: levelError } = await supabase
        ...
        .select(
          "id, code, name, min_age, max_age, uk_equivalent, us_canada_equivalent, nigeria_teacher_match",
        )
        .eq("is_active", true)
    ...
    loadData();
  }, [bookingMode, bookingNextPath, router]);
  ...
  const classOptions =
    classOptionsByCountry[countrySystem] ?? classOptionsByCountry.Other;
  ...
  async function handleAddStudent(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (!selectedLearningLevel) {
      setMessage(
        "Age is valid, but no matching learning level was found. Please check that EY, LP and UP are active in learning_levels.",
      );
      return;
    }
    ...
    if (bookingMode && createdStudent?.id) {
      router.push(subjectsPath(createdStudent.id));
      return;
    }
      ...
      <div className="container">
        ...
        >
          <section className="card" style={{ padding: 32 }}>
            ...
            >
              {bookingMode ? "Booking step 2: Child" : "Student Setup"}
            </p>

            <h1 className="page-title" style={{ fontSize: 38, marginTop: 10 }}>
              {bookingMode ? "Add or choose a child" : "Add a child"}
            </h1>

            <p className="page-subtitle">
              {bookingMode
                ? "Choose an existing child, or add one now. Your next screen will be subject selection."
            ...
            >
              ...
              {selectedLearningLevel ? (
                <div className="panel" style={{ padding: 18 }}>
                  ...
                  <p className="page-subtitle" style={{ marginTop: 8 }}>
                    UK: {selectedLearningLevel.uk_equivalent || "-"} •
                    US/Canada:{" "}
                    {selectedLearningLevel.us_canada_equivalent || "-"} •
                    Nigeria teacher match:{" "}
                ...
                <div className="panel" style={{ padding: 18 }}>
                  <p style={{ margin: 0, fontWeight: 800 }}>
                    Learning level pending
                  </p>
                  <p className="page-subtitle" style={{ marginTop: 8 }}>
                    Select age 3–11 to map your child to Early Years, Lower
                    Primary, or Upper Primary.
                  </p>
                </div>
              )}
              ...
              <button className="btn-primary" disabled={saving}>
                {saving
                  ? "Saving..."
                  : bookingMode
                    ? "Add Child & Continue to Subjects"
                    : "Add Child"}
              </button>
            </form>
          ...
          <aside className="card" style={{ padding: 28 }}>
            ...
            {students.length === 0 ? (
              <p className="page-subtitle">
                No child profile added yet. Add your first child to choose
                subjects.
              </p>
            ) : (
